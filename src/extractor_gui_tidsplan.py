...
Denne variant (extractor_gui_tidsplan.py) bygger desuden tidsplan.py ind som
en ekstra knap, så den kan afprøves uden at røre den almindelige
extractor_gui.py. Kræver Python + openpyxl + Pillow + python-pptx -
tkinter følger med Python.
"""
...
class App:
    def __init__(self, root):
        ...
        self._gruppe(krop, "TIDSPLAN", [
            ("Byg visuel tidsplan (PowerPoint)", self.tidsplan_byg),
        ])
    ...
    def kommentarer(self):
        ...
        if sprunget_over:
            messagebox.showwarning(
                ...
                f"{len(sprunget_over)} dokument(er) kunne ikke læses og "
                "blev sprunget over:\n\n" + "\n\n".join(sprunget_over))
        sti = self.gem_som(f"Emnelog over kommentarer {ex.dansk_dato()}.xlsx")
        if not sti:
            return
    ...
    def kommentarer_filtreret(self):
        ...
        if not recs:
            ...
            return
        sti = self.gem_som(f"Emnelog over kommentarer (filtreret) {ex.dansk_dato()}.xlsx")
        if not sti:
            return
        ex.write_simple(recs, ["Dokument", "Nummer", "Tråd", "Type", "Svar på",
                               "Dato", "Kommentar", "Markeret tekst", "Initialer",
    ...
    def tidsplan_byg(self):
        try:
            import tidsplan as tp
        except ImportError:
            messagebox.showerror("Mangler modul",
                                 "tidsplan.py skal ligge i samme mappe som denne fil.")
            return
        valgt = filedialog.askopenfilename(
            title="Vælg Excel-udtræk fra MS Project",
            filetypes=[("Excel-filer", "*.xlsx")])
        if not valgt:
            self.sæt_status("Ingen fil blev valgt.")
            return
        fil = self._lang_fil(valgt)
        dato_tekst = simpledialog.askstring(
            "Statusdato", "Statusdato for tidsplanen (ÅÅÅÅ-MM-DD):",
            initialvalue=dt.date.today().isoformat(), parent=self.root)
        if not dato_tekst:
            self.sæt_status("Annulleret - ingen statusdato angivet.")
            return
        try:
            idag = dt.date.fromisoformat(dato_tekst.strip())
        except ValueError:
            messagebox.showerror("Ugyldig dato",
                                 "Statusdatoen skal skrives som ÅÅÅÅ-MM-DD, fx 2026-09-16.")
            return
        self.sæt_status(f"Læser {fil.name} ...")
        try:
            lanes = tp.laes_plan(fil)
            prims, advarsler = tp.byg(lanes, idag)
        except Exception:
            messagebox.showerror("Fejl ved indlæsning",
                                 "Kunne ikke danne tidsplanen ud fra den valgte fil:\n\n"
                                 + traceback.format_exc())
            return
        sti = filedialog.asksaveasfilename(
            title="Gem tidsplan som", initialfile=f"tidsplan_visuel {ex.dansk_dato()}.pptx",
        ...
        try:
            ...
            messagebox.showerror("Fejl ved tegning",
                                 "Kunne ikke gemme PowerPoint-filen:\n\n" + traceback.format_exc())
            return
        besked = f"Tidsplan gemt:\n{sti}"
        if advarsler:
            besked += "\n\nADVARSLER:\n" + "\n".join(advarsler)
        self.sæt_status(f"Tidsplan gennemført -> {Path(sti).name}")
