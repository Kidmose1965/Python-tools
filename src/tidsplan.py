"""
Tidsplan - visuel udbudstidsplan fra MS Project-Excel-udtraek.
Bygget-i variant af Extractor-applikationen: bruger python-pptx direkte til
at tegne pptx'en (ingen Node/pptxgenjs/draw.js).

Brug som selvstaendigt script:
    python tidsplan.py <plan.xlsx> --idag 2026-09-10 [-o tidsplan_visuel.pptx]
...
Kraever: Python (openpyxl, Pillow, python-pptx).
"""
...
# =====================================================================
# Rendering (erstatter den tidligere draw.js/Node/pptxgenjs-pipeline)
# =====================================================================
def _rgb(hex_str):
    return RGBColor.from_string(hex_str)


def _bland_med_hvid(hex_str, transparency_pct):
    """Erstatning for aegte alpha-gennemsigtighed (som python-pptx ikke
    understoetter via det offentlige API): blander farven med hvid i samme
    forhold, saa resultatet visuelt svarer til en lys skravering."""
    r, g, b = int(hex_str[0:2], 16), int(hex_str[2:4], 16), int(hex_str[4:6], 16)
    a = transparency_pct / 100
    r, g, b = (round(v * (1 - a) + 255 * a) for v in (r, g, b))
    return f"{r:02X}{g:02X}{b:02X}"


def _uden_ramme_og_skygge(shape):
    shape.line.fill.background()
    shape.shadow.inherit = False


def _skriv_tekst(text_frame, s, align, valign, wrap):
    text_frame.word_wrap = bool(wrap)
    text_frame.margin_left = text_frame.margin_right = 0
    text_frame.margin_top = text_frame.margin_bottom = 0
    if valign == "middle":
        text_frame.vertical_anchor = MSO_ANCHOR.MIDDLE

    # Byg (tekst, stil)-par; en plan streng bliver ét par med hele prim-dictets
    # egne noegler som stil (size/bold/italic/color, sat af kaldende text()).
    par_align = PP_ALIGN.CENTER if align == "center" else PP_ALIGN.RIGHT if align == "right" else None

    def paragraffer(runs):
        rows = [[]]
        for tekst, stil in runs:
            for i, del_ in enumerate(tekst.split("\n")):
                if i > 0:
                    rows.append([])
                if del_:
                    rows[-1].append((del_, stil))
        return rows

    for i, para_runs in enumerate(paragraffer(s)):
        p = text_frame.paragraphs[0] if i == 0 else text_frame.add_paragraph()
        if par_align is not None:
            p.alignment = par_align
        for tekst, stil in para_runs:
            r = p.add_run()
            r.text = tekst
            r.font.name = "Calibri"
            r.font.size = Pt(stil.get("size", 10))
            r.font.bold = bool(stil.get("bold", False))
            r.font.italic = bool(stil.get("italic", False))
            r.font.color.rgb = _rgb(stil.get("color", "000000"))


def render(prims, sti):
    """Tegner primitiverne fra byg() direkte som pptx-figurer."""
    prs = Presentation()
    ...
    for p in prims:
        t = p["t"]
        if t == "rect":
            x, y, w, h = emne(p["x"], p["y"], max(p["w"], 0.001), max(p["h"], 0.001))
            fill = _bland_med_hvid(p["fill"], p["transparency"]) if "transparency" in p else p["fill"]
            shape = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, x, y, w, h)
            shape.fill.solid(); shape.fill.fore_color.rgb = _rgb(fill)
            _uden_ramme_og_skygge(shape)
        elif t == "diamond":
            x, y, w, h = emne(p["x"], p["y"], p["w"], p["h"])
            shape = slide.shapes.add_shape(MSO_SHAPE.DIAMOND, x, y, w, h)
            shape.fill.solid(); shape.fill.fore_color.rgb = _rgb(p["fill"])
            _uden_ramme_og_skygge(shape)
        elif t == "tri":
            x, y, w, h = emne(p["x"], p["y"], p["w"], p["h"])
            shape = slide.shapes.add_shape(MSO_SHAPE.ISOSCELES_TRIANGLE, x, y, w, h)
            shape.rotation = 90  # peger hoejre, som i den godkendte skabelon
            shape.fill.solid(); shape.fill.fore_color.rgb = _rgb(p["fill"])
            _uden_ramme_og_skygge(shape)
        elif t == "arrowhead":
            x, y, w, h = emne(p["x"], p["y"], p["w"], p["h"])
            ...
            x1, y1 = p["x"], p["y"]
            x2, y2 = x1 + p["w"], y1 + p["h"]
            conn = slide.shapes.add_connector(MSO_CONNECTOR.STRAIGHT, *emne(x1, y1, x2, y2))
            conn.line.color.rgb = _rgb(p["color"])
            conn.line.width = Pt(p.get("width", 0.75))
            if p.get("dash"):
                conn.line.dash_style = MSO_LINE_DASH_STYLE.DASH
        elif t == "text":
            x, y, w, h = emne(p["x"], p["y"], max(p["w"], 0.01), max(p["h"], 0.01))
            box = slide.shapes.add_textbox(x, y, w, h)
            s = p["s"]
            runs = s if isinstance(s, list) else [(s, {k: p[k] for k in ("size", "bold", "italic", "color") if k in p})]
            _skriv_tekst(box.text_frame, runs, p.get("align", "left"), p.get("valign", "top"), p.get("wrap", False))
        else:
            raise ValueError(f"Ukendt primitiv: {t}")
